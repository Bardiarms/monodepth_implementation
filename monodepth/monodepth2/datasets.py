...
class KITTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        left_raw = TF.to_tensor(left_img)
        right_raw = TF.to_tensor(right_img)

        # ImageNet normalization for ResNet encoder
        mean = [0.485, 0.456, 0.406]
        std  = [0.229, 0.224, 0.225]
        left = TF.normalize(left_raw, mean=mean, std=std)
        right = TF.normalize(right_raw, mean=mean, std=std)
